	...
	if not unit:character_damage():dead() and inventory then
		...
		if picked_up then
			...
			if not self._projectile_id and not self._weapon_category then
				...
				if not unit:character_damage():is_downed() and player_manager:has_category_upgrade("player", "gain_life_per_players") then
					if not player_manager:has_activate_temporary_upgrade("temporary", "loose_ammo_restore_health") then
						...
						if player_manager:has_category_upgrade("temporary", "loose_ammo_crit_bonus") then
							player_manager:add_gambler_crit_stack()
						end
						...
						if values ~= 0 then
							...
							restore_value = restore_value * (tweak_data.upgrades.loose_ammo_restore_health_values.multiplier or 0.1) * 2
							...
							if not damage_ext:need_revive() and not damage_ext:dead() and not damage_ext:is_berserker() then
								...
								if managers.player:has_category_upgrade("player", "loose_ammo_restore_armor") then
					...
					elseif managers.player:has_category_upgrade("player", "loose_ammo_restore_health_alt") then
